...
def preprocess_phrase(data,  heights, widths, area_frac_thresh, pred_list=[],verbose=True):
    """ preprocess phrase and remove phrase without predicate in 50 pred_list. """
    removed = 0
    total = 0
    for i, img in enumerate(data):
        area = float(heights[i] * widths[i])
        regions_copy = img['regions'][:]
        for region in regions_copy:
            # filter boxes by a box area-image area ratio threshold
            if float(region['height'] * region['width']) > area * area_frac_thresh:
                has_pred = False
                total += 1
                ...
                if len(phrase) > 0:
                    phrase = str(phrase).lower().translate(string.punctuation)
                    if pred_list:
                        for pred in pred_list:
                            if ' '+pred+' ' in phrase:
                                has_pred = True
                                break
                    if not pred_list or has_pred:
                        region['phrase'] = phrase.encode('utf-8', 'ignore')
                    else:
                        # filter phrase by if it contains any predicate
                        img['regions'].remove(region)
                        removed += 1
                else:
                    # filter phrase by len of phrase
                    img['regions'].remove(region)
                    removed += 1
            else:
                # filter boxes by a box area-image area ratio threshold
        ...
        #break
    if verbose:
        print('Keeping %d / %d phrase with predicate' % (total - removed, total))
...
def merge_duplicate_boxes(data, target_type):
    ...
    def overlap(obj1, obj2):
        ...
        if _target_type == 'obj':
            if all(b1 == b2) or iou > 0.9:  # consider as the same box
                ...
                return 0  # no overlap
        if _target_type == 'phrase':
            if  iou > 0.7:  # consider as the same box
                return 1 # no to motify the bbox
            elif iou > 0.5:
                return 2  # no to motify the bbox
            else:
                return 0  # no overlap
    ...
    print('merging {} boxes..'.format(target_type))

    lock = Lock()
    q = Queue()
    for i, img in enumerate(data):
        q.put((i, img))
        #break todo: only process one image
    def worker():
        while True:
            i, img = q.get()

            if i % 10000 == 0:
                print('processing %i/%i images...' % (i, len(data)))
            ...
            for i in range(num_obj):
                ...
                objs[i]['mobjs'] = merged_objs  # all similar bboxs
            ...
            for obj in objs:
                if 'M_TYPE' not in obj:
                    ...
                    if target_type == 'obj':
                        obj['names'] = [*{*obj['names']}]  # remove duplicates, name will be a list contain all kind possible name of this bbox
                    if target_type == 'phrase':
                        try:
                            obj['phrase'] = [*{*obj['phrase']}]#list(set(obj['phrase']))  # remove duplicates
                        except:
                            print("obj['phrase'] {}".format(obj['phrase']))
...
def encode_box(region, org_h, org_w, im_long_size):
    ...
    # todo: not suit, the im_size is not square, some w or height maybe smaller than image_size.
    # so there is possibility of whole bbox out of img
    if x > image_size - 2:
        x = image_size - 2
    if y > image_size - 2:
        y = image_size - 2
...
def encode_phrases(phreses_data):
    """MUST BE CALLED AFTER encode_objects!!!"""
    encoded_phrase = []  # encoded phrase

    im_to_first_phrase = np.zeros(len(phreses_data), dtype=np.int32)
    im_to_last_phrase = np.zeros(len(phreses_data), dtype=np.int32)
    phrases_idx_counter = 0
    no_phrase_counter = 0

    for i, img in enumerate(phreses_data):
        im_to_first_phrase[i] = phrases_idx_counter
        ...
        for reg in img['regions']:
            ...
            if len(phrase) >1 :
                xx = ','.encode('utf-8')
                phrase = [xx.join(phrase)]
                encoded_phrase.append(phrase)
                phrases_idx_counter += 1  # accumulate counter

        if im_to_first_phrase[i] == phrases_idx_counter:
            # if no qualifying relationship
            im_to_first_phrase[i] = -1
            im_to_last_phrase[i] = -1
            no_phrase_counter += 1
        else:
            im_to_last_phrase[i] = phrases_idx_counter - 1


    print('%i phrases remains ' % len(encoded_phrase))

    return np.vstack(encoded_phrase), im_to_first_phrase, im_to_last_phrase
...
def sentence_preprocess(phrase):
    ...
    return str(phrase).lower().translate(string.punctuation)#.decode('utf-8', 'ignore')
...
def filter_object_boxes(data, heights, widths, area_frac_thresh):
    ...
    print('obj box threshod: keeping %i/%i boxes' % (thresh_count, all_count))
...
def main(args):
    ...
    # add by my self to load phrase of vg
    phrase_data = json.load(open(args.region_descriptions))
    ...
    assert(len(rel_data) == len(obj_data) and
          len(obj_data) == len(img_data) and
          len(phrase_data) == len(img_data))
    ...
    num_im -= 4
    for c_im in corrupted_ims:
        index = np.argwhere(valid_im_idx == c_im)
        valid_im_idx = np.delete(valid_im_idx, index)

    phrase_data = filter_by_idx(phrase_data, valid_im_idx)
    ...
    img_ids = filter_by_idx(img_ids, valid_im_idx)
    #sanity check
    ...
    if args.load_frac < 1:
        ...
        phrase_data = phrase_data[:num_im]
    ...
    preprocess_phrase(phrase_data, heights, widths, 2*args.min_box_area_frac, pred_list) #0.002x2

    if args.min_box_area_frac > 0: #0.002
    ...
    merge_duplicate_boxes(obj_data, 'obj')
    merge_duplicate_boxes(phrase_data, 'phrase')



    # build vocabulary 150
    ...
    encoded_phrase, im_to_first_phrase, im_to_last_phrase = encode_phrases(phrase_data)
    f.create_dataset('phrases', data=encoded_phrase)
    f.create_dataset('im_to_first_phrase', data=im_to_first_phrase)
    f.create_dataset('im_to_last_phrase', data=im_to_last_phrase)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--region_descriptions', default='VG/region_descriptions.json', type=str)
   ...
   #  pp = np.vstack(p)
    # # f = h5.File(args.h5_file, 'w')
    #  cc = c[0].decode('utf-8').split(',')
    #
    #
    #  f.create_dataset('labels', data=pp)
