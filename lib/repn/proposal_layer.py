...
class _ProposalLayer(nn.Module):
    ...
    def forward(self, input, use_gt_boxes=False):
        ...
        proposals = proposals.contiguous().view(-1, 2)[keep.nonzero().squeeze(), :].contiguous().view(batch_size, -1, 2).contiguous()
        ...
        all_box_pairs_score = []
        post_nms_topN = proposals.size(1) #todo with/o proposal, during training if we comment out this line, res is bad.
        ...
        for b in range(batch_size):#batch_size
            ...
            box_pairs_fet = (torch.cat((proposals_subject_fet_i, proposals_object_fet_i), 0)).view(2,-1, self.sub_feat_size)
        ...
        #     scores_i = all_box_pairs_score[b]
            ...
            _, order_i = torch.sort(scores_i,descending=True)# from big to small
            ...
            if not use_gt_boxes:
                ...
                keep_idx_i = nms(rel_rois_final, scores_single.squeeze(1), 1).long().view(-1)
            ...
            output[b,:num_proposal,1:5] = rois[b][proposals_single[:, 0], :][:, 1:5]
            output[b,:num_proposal,5:] = rois[b][proposals_single[:, 1], :][:, 1:5]
