...
from .bbox_transform import bbox_transform, bbox_overlaps, co_bbox_overlaps_batch
import pdb
...
class _RelProposalTargetLayer(nn.Module):
    ...
    def forward(self, roi_pairs, gt_boxes, gt_relation, num_boxes):
        ...
        for i in range(batch_size):
            ...
            try:
                ...
                gt_box_pairs[i][:n_rel, 8] = gt_pairs_i[:n_rel, 2]
            except:
                print('wrong in relpn target layer ??????????????????')
    ...
    def _sample_roi_pairs_pytorch(self, all_roi_pairs, gt_box_pairs, fg_rois_per_image, rois_per_image, num_classes):
        ...
        overlaps = co_bbox_overlaps_batch(all_roi_pairs[:,:,1:].contiguous(),
        ...
        # change the index way of pytorch 1.0
        labels = gt_box_pairs[:,:,8].contiguous().view(-1)[(offset.view(-1),)].view(batch_size, -1)
